...
\begin{prepa}
La cadence affichée en bas à droite (par exemple \texttt{9.5 Hz}) est celle
de la \emph{séquence complète}, pas celle d'une commande. Chaque séquence
envoie quatre commandes ; à \SI{9,5}{\hertz}, ce sont donc environ
\num{38} commandes par seconde qui circulent sur le câble. La cadence
...
perturbation observable : la moitié de la cadence. À \SI{9,5}{\hertz}, aucune
vibration au-delà de \SI{4,75}{\hertz} n'est visible dans les données de
...
\item[\texttt{RA: 02:44:08.67}] La position brute renvoyée par la monture,
telle quelle, en heures : minutes : secondes. C'est la réponse à la commande
\texttt{:GR\#}, sans retouche.

\item[\texttt{$\Delta$ +0.29\arcsec}] L'écart à la référence, en secondes
d'arc. Ce nombre devient rouge dès qu'il dépasse la tolérance que vous avez
fixée.

\item[\texttt{$\sigma$ 0.16\arcsec}] L'écart-type glissant : la dispersion des
dernières minutes. C'est le chiffre qui compte vraiment --- voir le
chapitre~\ref{ch:ad}.

\item[\texttt{9.5 Hz | 879 samples}] La cadence réellement obtenue, et le
nombre d'échantillons depuis le début de la session.
\end{description}

\begin{warning}
\texttt{879 samples} compte les échantillons \emph{depuis le début}, pas ceux
...
\texttt{$\Delta$ +0.019s} au lieu de \texttt{$\Delta$ +0.29\arcsecond}, et les
...
\SI{9,5}{\hertz} et $\sigma_x = \SI{0,1}{\arcsecond}$, cela fait déjà
...
d'échantillonnage. À \SI{9,5}{\hertz}, rien au-delà de \SI{4,75}{\hertz}.
